...
#define BALANCE_BRAKE_PREDICT_TIME_S      0.50f
#define BALANCE_BRAKE_MIN_VELOCITY       90.0f
#define BALANCE_BRAKE_RELEASE_VELOCITY   60.0f
#define BALANCE_BRAKE_HOLD_FRAMES            2
#define BALANCE_BRAKE_MIN_TARGET_STEPS  600.0f
#define BALANCE_BRAKE_TAPER_PIXELS       50.0f
#define BALANCE_BRAKE_MIN_TARGET_NEAR_STEPS 180.0f
...
static void Balance_UpdateTargetPosition(float dt_s)
{
    ...
    else
    {
        ...
        /* Use strong braking far away, then reduce the forced minimum near center. */
        taper_ratio = ClampFloat(abs_error / BALANCE_BRAKE_TAPER_PIXELS,
                                 0.0f, 1.0f);
        brake_min_target_steps =
            BALANCE_BRAKE_MIN_TARGET_NEAR_STEPS +
            (BALANCE_BRAKE_MIN_TARGET_STEPS -
             BALANCE_BRAKE_MIN_TARGET_NEAR_STEPS) * taper_ratio;
    }
